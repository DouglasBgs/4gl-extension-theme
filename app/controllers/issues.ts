...
export class Issues {
    ...
    if (!nome_issue || !await Utils.validaDadosInformados(nome_issue)) {
      ...
      switch (selecionado) {
        ...
        case Etapa.TesteUnitario:
          ...
          Diretorios.copiaPasta(config.data.arquivos_romana + nome_issue, config.data.issue_tu + nome_issue, true)
          break
        case Etapa.TesteIntegrado:
          Diretorios.criar(nome_issue, config.data.issue_ti, Etapa.TesteIntegrado)
          Diretorios.copiaPasta(config.data.arquivos_romana + nome_issue, config.data.issue_ti + nome_issue, true)
          break
        default:
          break
      }
  ...
  private static async openTomcat(config: ConfigModel, nome_issue: string) {
   let exists = await Diretorios.BuscaArquivos(`${config.data.compilado_datasul}${nome_issue}`)
    ...
    await Diretorios.copiaPasta(`${config.data.compilado_datasul}${nome_issue}`, `${config.data.tomcat_datasul}webapps`).then(() => {
      Utils.MostraMensagemInfo(`Arquivos copiados com sucesso para o tomcat: ${config.data.tomcat_datasul}`)
    })
